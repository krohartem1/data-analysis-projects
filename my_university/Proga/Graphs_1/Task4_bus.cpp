...
int64_t algorithm_Dijkstra(std::map<int, std::vector<std::tuple<int, int, int>>>& graph, int S, int F)
{
	...
	std::vector<int> dist(graph.size() + 1, L);
	dist[S] = 0;
	...
	if (!graph[S].empty())
	{
		while (change)
		{
			...
			if (visited[next_vertex]) { pq.pop(); }
				...
				{
					if (dist[next_vertex] <= std::get<0>(graph[next_vertex][i]) &&  dist[std::get<1>(graph[next_vertex][i])] > std::get<2>(graph[next_vertex][i]))
					{
						dist[std::get<1>(graph[next_vertex][i])] = std::min(std::get<2>(graph[next_vertex][i]), dist[std::get<1>(graph[next_vertex][i])]);

...
int max_length(std::vector<long long int>& powx, std::vector<long long int>& hash, int a, int b, int p, int size)
{
	int sum = 0;
	if (b == size) {
		if (substr_equality(powx, hash, 1, a, b, p)) { sum = 1; return sum; }
		else
		{
			return sum;
		}
	...
	int k = ((size - b + 1) / 2);
	if (substr_equality(powx, hash, k, a, b, p)) {
		sum += k + max_length(powx, hash, a + k, b + k, p, size);
		
	}
	else {
		sum += max_length(powx, hash, a, b, p, b + k - 1);
	}
	return sum;
}
int main()
{
	...
	std::vector<int> z(s0.size());
	z[0] = 0;
	for (size_t i = 1; i < s0.size(); ++i)
	{
		z[i] = std::min(max_length(powx1, h1, 1, i + 1, p1, s0.size()),max_length(powx2, h2, 1, i + 1, p2, s0.size()));
	}
